...
class AgenticDesigner:
    ...
    async def analyze_website(
        ...
        try:
            ...
            print(f"📊 Multi-stage analysis: {'enabled' if multi_stage else 'disabled'}")
            print(f"📱 Mobile analysis: {'enabled' if include_mobile else 'disabled'}")
            print("-" * 60)
    ...
    def _print_analysis_summary(self, analysis: dict):
        ...
        if "Typography" in analysis:
            ...
            if isinstance(typography, dict):
                fonts = typography.get("Primary, secondary & fallback font families (web-safe or web-hosted)", [])
                if fonts:
                    print(f"🔤 Primary Font: {fonts[0] if isinstance(fonts, list) else fonts}")
        
        # Color summary
        if "Color & Contrast" in analysis:
            colors = analysis["Color & Contrast"]
            if isinstance(colors, dict):
                primary_colors = colors.get("Brand primaries, secondaries, accents (hex/RGB/HSL)", {})
                if primary_colors:
        ...
        if "Layout & Grid System" in analysis:
            ...
            if isinstance(layout, dict):
                max_width = layout.get("Maximum content width / full-bleed rules", "")
                if max_width:
        ...
        if "Tone & Mood Descriptors" in analysis:
            ...
            if isinstance(mood, dict):
                adjectives = mood.get("Three-to-five adjectives defining the visual vibe (e.g., 'clean, approachable, tech-forward')", [])
                if adjectives:
