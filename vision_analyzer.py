    ...
    async def analyze_screenshot(
        ...
        try:
            ...
            if hasattr(self, '_last_successful_merge'):
                print(
                    f"DEBUG: Using last successful merge due to error: {str(e)}")
    ...
    def _parse_response(self, response_text: str) -> Dict[str, Any]:
        ...
        try:
            ...
            while '```json' in text:
                ...
                if end != -1:
                    ...
                    try:
                        ...
                        print(
                            f"DEBUG: Found JSON chunk: {list(chunk.keys())[:3]}...")
            ...
            if json_chunks:
                ...
                print(
                    f"DEBUG: Successfully merged {len(json_chunks)} JSON chunks")
    ...
    def _merge_analysis_chunks(self, chunks: List[Dict]) -> Dict[str, Any]:
        ...
        for chunk in chunks:
            ...
            if 'typography' in chunk:
                merged_analysis['Typography'] = self._map_typography(
                    chunk['typography'])
            ...
            if 'gridSystem' in chunk or 'spacingPatterns' in chunk:
                merged_analysis['Layout & Grid System'] = self._map_layout(
                    chunk)

            # Button/component mapping
            if 'buttonStyles' in chunk:
                merged_analysis['Buttons & Calls-to-Action'] = self._map_buttons(
                    chunk['buttonStyles'])
        ...
        print(
            f"DEBUG: Merged analysis with {len(merged_analysis)} categories from {len(chunks)} total chunks")
    ...
    def _map_layout(self, layout_data: Dict) -> Dict:
        ...
        return {
            "Maximum content width / full-bleed rules":
                layout_data.get('containerWidths', {}).get(
                    'extraLarge', 'Not specified'),
    ...
    async def analyze_website(
        ...
        async with ScreenshotService() as screenshot_service:
            ...
            if include_mobile:
                ...
                # Optimize images
                desktop_screenshot = ScreenshotService.optimize_image(
                    desktop_screenshot)
                mobile_screenshot = ScreenshotService.optimize_image(
                    mobile_screenshot)
                ...
                screenshot_path = f"outputs/screenshots/{url.replace('https://', '').replace('http://', '').replace('/', '_')}.png" if save_screenshot else None
